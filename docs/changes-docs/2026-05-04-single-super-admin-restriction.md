...
## Database Schema
No database schema changes were required. The application-level logic handles the restriction entirely.
...
2. Deactivate other super admin assignments:
   ```sql
   UPDATE user_role_assignments 
   SET is_active = 0 
   WHERE role_name = 'super_admin' AND user_id NOT IN (selected_super_admin_id);
   ```
3. Or delete them:
   ```sql
...
- Error handling ensures graceful degradation if API call fails
